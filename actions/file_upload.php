...
function file_upload($picture) {
   ...
   if ($fileError == 4) {      
       ...
       if (in_array($fileExtension, $filesAllowed)) {
           if ($fileError === 0) {
               if ($fileSize < 5000000) { //5Mb this number is in bytes
                   ...
                   $result->ErrorMessage = "This picture is bigger than the allowed 5Mb. <br> Please choose a smaller one and update the product.";
                   return $result;
               }
           } else {                              
